...
public final class edit_005fopportunity_jsp extends org.apache.jasper.runtime.HttpJspBase
  ...
  public void _jspService(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      out.write("            ");
            ...
            Statement stmtRadio = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);    
               ...
               // query for type
               String tQuery = "select type from opportunity where id = "+ oppId +"";
               ResultSet rsRadio = null;
      ...
      out.write("\t\t\t\t\t\t\t\r\n");
      out.write("\t\t\t\t\t\t\t\t<br></br>\r\n");
      out.write("\t\t\t\t\t\t\t\t\t <form name=\"form\" onsubmit=\"//return validateFirstForm();\" role=\"form\" method=\"POST\" action=\"editOpportunity\">\r\n");
      out.write("\t\t\t\t\t\t\t\t\t\r\n");
      out.write("                                                                             ");
 ...
 while(rsOpp.next()) {
      ...
      out.write("\t\t\t\t\t\t\t\t\t\t");
                                                                                ...
                                                                                String oName="",type="", description="", start_date="", 
                                                                                        end_date="", cPerson="", cNumber="", cLandline="", eadd="";
                                                                                
                                                                                if (session.getAttribute("oppName")!=null&&rsOpp.getString("name")==null)
                                                                                    oName= String.valueOf(session.getAttribute("oppName"));
                                                                                else
                                                                                    oName = rsOpp.getString("name");
                                                                                if (session.getAttribute("oppType")!=null&&rsOpp.getString("type")==null)
                                                                                    type= String.valueOf(session.getAttribute("oppType"));
                                                                                else
                                                                                    type = rsOpp.getString("type");
                                                                                if (session.getAttribute("oppDescription")!=null&&rsOpp.getString("description")==null)
                                                                                    description= String.valueOf(session.getAttribute("oppDescription"));
                                                                                else
                                                                                    description = rsOpp.getString("description");
                                                                                if (session.getAttribute("oppStartDate")!=null&&rsOpp.getString("start_date")==null)
                                                                                    start_date= String.valueOf(session.getAttribute("oppStartDate"));
                                                                                else
                                                                                    start_date = rsOpp.getString("start_date");
                                                                                if (session.getAttribute("oppEndDate")!=null&&rsOpp.getString("end_date")==null)
                                                                                    end_date= String.valueOf(session.getAttribute("oppEndDate"));
                                                                                else
                                                                                    end_date = rsOpp.getString("end_date");
                                                                                if (session.getAttribute("oppContactPerson")!=null&&rsOpp.getString("contact_person")==null)
                                                                                    cPerson= String.valueOf(session.getAttribute("oppContactPerson"));
                                                                                else
                                                                                    cPerson = rsOpp.getString("contact_person");
                                                                                if (session.getAttribute("oppPhone")!=null&&rsOpp.getString("contact_number")==null)
                                                                                    cNumber= String.valueOf(session.getAttribute("oppPhone"));
                                                                                else
                                                                                    cNumber = rsOpp.getString("contact_number");
                                                                                if (session.getAttribute("oppLandline")!=null&&rsOpp.getString("contact_landline")==null)
                                                                                    cLandline= String.valueOf(session.getAttribute("oppLandline"));
                                                                                else
                                                                                    cLandline = rsOpp.getString("contact_landline");
                                                                                if (session.getAttribute("oppEmail")!=null&&rsOpp.getString("email")==null)
                                                                                    eadd= String.valueOf(session.getAttribute("oppEmail"));
                                                                                else
                                                                                    eadd = rsOpp.getString("email");
      ...
      out.write("                                                                                    ");
                                                                                     ...
                                                                                     rsRadio = stmtRadio.executeQuery(tQuery);
                                                                                     
                                                                                        while(rsRadio.next()) {
                                                                                            if (session.getAttribute("oppType")!=null&&rsRadio.getString("type")==null)
                                                                                                strRadio= String.valueOf(session.getAttribute("oppType"));
                                                                                            else
                                                                                                strRadio = rsRadio.getString("type");
                                                                                        }
      ...
      out.write("                                                                                                         \r\n");
      out.write("                                                                                    <input type=\"radio\" name=\"type\" value=\"Academe\" required ");
      out.print(acadRadio);
      out.write("><label for=\"type\">Academe</label>\r\n");
      out.write("                                                                                    <input type=\"radio\" name=\"type\" value=\"Company\"");
      out.print(compRadio);
      out.write("><label for=\"type\">Company</label>\r\n");
      out.write("\t\t\t\t\t\t\t\t\t\t\t<label for=\"company\">Academe/Company name:</label>\r\n");
      out.write("\t\t\t\t\t\t\t\t\t\t\t<input name=\"AoCType\" type=\"text\" value=\"\" class=\"form-control\" id=\"inputCompany\">\r\n");
 ...
 String cSelect="", country="";
                                                                                        while (rs.next()) {
                                                                                            if (session.getAttribute("oppCountry")!=null&&rs.getString("name")==null) {
 ...
 cSelect="";
      ...
      out.write("                                                                                        ");
                                                                                    ...
                                                                                    if (session.getAttribute("oppColleges") != null&&s.toString()==null)
      ...
      out.write("                    var minDatePlus = new Date(minDate.setMonth(minDate.getMonth()+1));\r\n");
      out.write("                    $('#eDate').datepicker('setStartDate',minDatePlus);\r\n");
      out.write("                    \r\n");
      out.write("                });  \r\n");
      out.write("            \r\n");
      out.write("            });\r\n");
